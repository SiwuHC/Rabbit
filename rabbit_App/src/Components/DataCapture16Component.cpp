...
DataCapture16RawComponent::DataCapture16RawComponent(QWidget *parent)
...
DataCapture16RawComponent::~DataCapture16RawComponent() {}

void DataCapture16RawComponent::reset() {
  captured_value_ = 0;
  has_captured_ = false;
...
void DataCapture16RawComponent::processReadData(QQueue<uint64_t> &read_queue) {
  if (read_queue.isEmpty()) {
    return;
  }

  // First port (index 0) is CAPTURE signal
  // Next 16 ports (index 1-16) are DOUT0-DOUT15 data bits
...
uint64_t DataCapture16RawComponent::getWriteData() const {
  return 0; // Output-only component
}

void DataCapture16RawComponent::paintEvent(QPaintEvent *event) {
...
void DataCapture16RawComponent::initPorts() {
  // First port is CAPTURE signal
  appendPort(output_ports_, "CAPTURE", ports::PortType::Output);
  // Next 16 ports are data bits: DOUT0 (LSB) to DOUT15 (MSB)
  ...
  appendPort(output_ports_, "DOUT8", ports::PortType::Output);
  appendPort(output_ports_, "DOUT9", ports::PortType::Output);
  appendPort(output_ports_, "DOUT10", ports::PortType::Output);
  appendPort(output_ports_, "DOUT11", ports::PortType::Output);
  appendPort(output_ports_, "DOUT12", ports::PortType::Output);
  appendPort(output_ports_, "DOUT13", ports::PortType::Output);
  appendPort(output_ports_, "DOUT14", ports::PortType::Output);
  appendPort(output_ports_, "DOUT15", ports::PortType::Output);
}

void DataCapture16RawComponent::updateDisplay() {
  status_label_->setText("Status: Captured");
  dec_label_->setText(QString("Dec: %1").arg(captured_value_));
  hex_label_->setText(
      QString("Hex: 0x%1").arg(captured_value_, 4, 16).toUpper());
}
