#include <QVBoxLayout>
#include <QPainter>

#include "Components/ComponentSettingsDialog.h"
#include "Components/AbstractComponent.h"
#include "Components/DataCapture32Component.h"

using namespace rabbit_App::component;

DataCapture32RawComponent::DataCapture32RawComponent(QWidget *parent)
    : AbstractRawComponent(parent), captured_value_(0), has_captured_(false) {
  initPorts();

  auto main_layout = new QVBoxLayout(this);
  main_layout->setContentsMargins(5, 5, 5, 5);
  main_layout->setSpacing(2);

  status_label_ = new QLabel("Status: Waiting...", this);
  dec_label_ = new QLabel("Dec: --", this);
  hex_label_ = new QLabel("Hex: --", this);

  status_label_->setStyleSheet("font-size: 12px; font-weight: bold;");
  dec_label_->setStyleSheet("font-size: 12px;");
  hex_label_->setStyleSheet("font-size: 11px;");

  main_layout->addWidget(status_label_);
  main_layout->addWidget(dec_label_);
  main_layout->addWidget(hex_label_);

  setLayout(main_layout);
}

DataCapture32RawComponent::~DataCapture32RawComponent() {}

void DataCapture32RawComponent::reset() {
  captured_value_ = 0;
  has_captured_ = false;
  status_label_->setText("Status: Waiting...");
  dec_label_->setText("Dec: --");
  hex_label_->setText("Hex: --");
}

void DataCapture32RawComponent::processReadData(QQueue<uint64_t> &read_queue) {
  if (read_queue.isEmpty()) {
    return;
  }

  // First port (index 0) is CAPTURE signal
  // Next 32 ports (index 1-32) are DOUT0-DOUT31 data bits
  auto capture_port_index = output_ports_[0].pin_index - 1; // -1 because index 0 is clock

  // Check the most recent sample for capture signal
  auto latest_value = read_queue.back();

  // Check if capture signal is high
  bool capture_high = (latest_value >> capture_port_index) & 0x1;

  if (capture_high) {
    // Capture the 32-bit data
    quint32 data = 0;
    for (auto i = 0; i < 32; ++i) {
      auto port_index = output_ports_[i + 1].pin_index - 1;
      data |= (((quint64)latest_value >> port_index) & 0x1) << i;
    }
    captured_value_ = data;
    has_captured_ = true;
    updateDisplay();
  }
}

uint64_t DataCapture32RawComponent::getWriteData() const {
  return 0; // Output-only component
}

void DataCapture32RawComponent::paintEvent(QPaintEvent *event) {
  QPainter painter(this);
  painter.setRenderHint(QPainter::Antialiasing);

  // Draw border
  painter.setPen(Qt::black);
  painter.setBrush(Qt::white);
  painter.drawRect(rect());
}

void DataCapture32RawComponent::initPorts() {
  // First port is CAPTURE signal
  appendPort(output_ports_, "CAPTURE", ports::PortType::Output);
  // Next 32 ports are data bits: DOUT0 (LSB) to DOUT31 (MSB)
  for (auto i = 0; i < 32; ++i) {
    appendPort(output_ports_, QString("DOUT%1").arg(i), ports::PortType::Output);
  }
}

void DataCapture32RawComponent::updateDisplay() {
  status_label_->setText("Status: Captured");
  dec_label_->setText(QString("Dec: %1").arg(captured_value_));
  hex_label_->setText(
      QString("Hex: 0x%1").arg(captured_value_, 8, 16).toUpper());
}

COMPONENT_CLASS_DEFINITION(DataCapture32, 3, 3)

void DataCapture32Component::onSettingsBtnClicked() {
  auto dialog =
      new ComponentSettingsDialogWithFeatures<SettingsFeature::ArrayPortMapping>(
          this, this);
  dialog->exec();
  delete dialog;
}
